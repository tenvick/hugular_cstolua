...
public class ScrollRectTable : MonoBehaviour
{
	...
	public int removeChild(ScrollRectItem item)
	{
		...
		if(i>=0)
		{
            if (onDataRemove == null) onDataRemove = PLua.instance.lua.lua.LoadString(DataRemoveStr, "onDataRemove");
	...
	public int insertData(object item,int index)
	{
		...
		if (onDataInsert == null) onDataInsert = PLua.instance.lua.lua.LoadString(DataInsertStr, "onDataInsert");
...
//				delt.x=rect.width*x+this.padding.x*2;
	...
	void preRender(ScrollRectItem item,int index)
	{
        ...
        if (this.onPreRender == null) onPreRender = PLua.instance.lua.lua.LoadString(PreRenderStr, "onPreRenderStr");
